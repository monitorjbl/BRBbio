SELECT pl.plate_name, pa.time_marker, 1-(3*(ns.ctrl + ps.ctrl))/(na.ctrl-pa.ctrl) as z_factor
	...
	JOIN avg_control ps ON ps.plate_id = pl.id AND ps.time_marker = pa.time_marker AND ps.control_type = 'positive_control'
	JOIN avg_control ns ON ns.plate_id = pl.id AND ns.time_marker = pa.time_marker AND ns.control_type = 'negative_control'
WHERE pl.run_id = ? 
